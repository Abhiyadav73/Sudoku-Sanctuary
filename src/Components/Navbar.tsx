...
export default function Navbar({onClose, onShowHowToPlay, onShowStats, onShowSettings}: Props) {
  return (
   <header className="bg-surface/20 backdrop-blur-[20px] full-width top-0 z-50 sticky shadow-sm">
        <div className="flex justify-between items-center px-10 py-6 w-full max-w-360 mx-auto">
          <div className="flex items-center gap-6">
            <button 
              onClick={onClose}
              className="text-primary hover:bg-surface-variant/50 transition-colors duration-300 pt-2 rounded-full scale-95 active:scale-90"
            >
              <span className="material-symbols-outlined">arrow_back</span>
            </button>
            <h1 className="font-headline text-2xl font-extrabold tracking-tight"><span className='bigbesty text-navtx'>Sudoku <span className="text-[#ff0099]">Sanctuary</span></span></h1>
          </div> 
