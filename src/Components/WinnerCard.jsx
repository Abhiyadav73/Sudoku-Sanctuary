...
const performanceMessages = [
  ...
  { maxTime: Infinity, message: "Completed! Finished strong. 💪", color: "text-red-400" }
];
  ...
  return (
    <div className="flex flex-col items-center gap-5 p-4 select-none">
      <div
        ...
        style={{
          backgroundImage: `url('${window.location.origin}/numwal.png')`
        }}
      >
        <div className="absolute inset-0 bg-slate-950/30 backdrop-blur-sm z-0" />
        <div className="relative z-10 w-full flex flex-col items-center">

          <span className={`px-3 py-1 text-xs font-semibold tracking-wider uppercase rounded-full border border-white/20 ${modeColor(mode)} text-white mb-6 transition-all duration-300 hover:bg-white/20 bg-${modeColor(mode)}`}> 
            {mode}
          </span>
          ...
          <div className="w-20 h-20 mb-4 flex items-center justify-center bg-linear-to-br from-indigo-500 to-purple-600 rounded-2xl shadow-indigo-500/30 shadow-lg transform rotate-45 hover:scale-[1.05] hover:animate-[shakeTrophy_0.1s_ease-in]">
            <div className="grid grid-rows-3 grid-cols-3 gap-1 w-10 h-10 -rotate-45">
              {Array.from({ length: 9 }).map((_, idx) => (
                <div
                  key={idx}
                  className={`w-2.5 h-2.5 rounded-full transition-all duration-500 ${idx % 2 === 0 ? "bg-white scale-100" : "bg-white/40 scale-90"
                    }`}
                />
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 text-slate-800 text-xs my-1 group cursor-pointer">
            <svg
              ...
              className="text-purple-600 transition-transform duration-300 group-hover:scale-125 group-hover:rotate-12"
            >
              <path d="M12 13v8" />
              <path d="M12 3v3" />
